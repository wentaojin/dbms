...
func (a *App) AppDisable() component.Cmder {
	return &AppDisable{App: a}
}

func (a *AppDisable) Cmd() *cobra.Command {
	c := &cobra.Command{
		Use:              "disable <cluster-name>",
		Short:            "Disable automatic enabling of DBMS clusters at boot",
		RunE:             a.RunE,
...
func (a *AppDisable) RunE(cmd *cobra.Command, args []string) error {
	...
	return a.Disable(clusterName, gOpt)
}
...
func (a *AppDisable) Disable(clusterName string, gOpt *operator.Options) error {
	...
	mg.Logger.Infof("Disabling cluster %s...", clusterName)
	...
	b := task.NewBuilder(mg.Logger).
		...
		).Func("DisableCluster", func(ctx context.Context) error {
	...
	mg.Logger.Infof("Disabled cluster `%s` successfully", clusterName)
	return nil
}
