...
func (e *RowChangedEvent) Delete(
	...
	metadatas []*metadata) (string, []interface{}, error) {
	...
	ms := e.metadata(schemaName, tableName, metadatas)
	if ms == nil {
	...
	case constant.DatabaseTypeOracle:
		...
		for c, p := range uniqColumnData {
			if ds, ok := ms.tableColumnsT[c]; ok {
				if tys, ok := oldColumnTypeT[c]; ok {
					if p != nil {
						switch {
						case stringutil.IsContainedStringIgnoreCase(MessageEventColumnStringCharacterDatatype, tys) && stringutil.IsContainedStringIgnoreCase(
							...
							continue
						case stringutil.IsContainedStringIgnoreCase(MessageEventColumnDatetimeCharacterDatatype, tys):
						...
						case stringutil.IsContainedStringIgnoreCase(MessageEventColumnYearCharacterDatatype, tys):
							if strings.EqualFold(ds.columnType, constant.BuildInOracleDatatypeDate) {
						...
						case stringutil.IsContainedStringIgnoreCase(MessageEventColumnTimeCharacterDatatype, tys):
							if strings.EqualFold(ds.columnType, constant.BuildInOracleDatatypeDate) {
						...
						case stringutil.IsContainedStringIgnoreCase(MessageEventColumnCharCharacterDatatype, tys):
...
func (e *RowChangedEvent) metadata(schemaName, tableName string, metadatas []*metadata) *metadata {
	var metadata *metadata
	for _, ms := range metadatas {
		if ms.schemaNameT == schemaName && ms.tableNameT == tableName {
			metadata = ms
			break
		}
	}
...
// ExtractSortCommitTs extract and sort all Commits
func (g *EventGroup) ExtractSortCommitTs() []uint64 {
...
// difference function: returns the elements in a that are not in b
